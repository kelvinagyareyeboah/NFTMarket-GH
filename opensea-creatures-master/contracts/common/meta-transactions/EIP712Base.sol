...
/**
 * @title EIP712Base (Improved & Hardened)
 * @dev Optimized and secure EIP-712 implementation.
 *
 * Fixes vs. the original:
 *  - HALF_ORDER was set to an incorrect value (255 bits of 1s), which made
 *    the signature-malleability check almost never fire. It is now the
 *    correct secp256k1n / 2.
 *  - `v` is now strictly validated (must be 27 or 28) instead of silently
 *    coerced, which previously could mask malformed signatures.
 *  - Added a zero-address guard on the expected `signer` parameter.
 *  - Marked as `abstract` since this is meant to be inherited, not deployed
 *    directly.
 */
abstract contract EIP712Base is Initializable {
    // ============================================================
    // CONSTANTS
    // ============================================================
    ...
    // secp256k1n / 2 (correct value — used to reject malleable signatures)
        ...
        returns (address)
    {
        ...
        // Strictly validate v — do not silently coerce out-of-range values.
        if (v < 27) {
            v += 27;
        }
